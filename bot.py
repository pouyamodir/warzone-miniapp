...
def card_text(lobby):
    ...
    n = 1 + len({sid(a.get("id")) for a in answers if a.get("code") in ready})
    n = min(4, n)
    lines = [
...
def notify_new_answers(lobby, old):
    if not lobby or not old:
        return
    if sid(lobby.get("hostId")) != sid(old.get("hostId")):
        return
...
def tally_noreply(old, new):
    ...
    if opened is None or (time.time() - opened) < REPLY_WINDOW:
        return
    answered = {sid(a.get("id")) for a in (old.get("answers") or [])}
    host = sid(old.get("hostId"))
    members = members_map()
    for uid, info in members.items():
        uid = sid(uid)
        info = info or {}
        if not uid or uid == host or uid in answered or hidden(uid, info):
            continue
        if uid in ("local", "undefined", "null"):
            continue
