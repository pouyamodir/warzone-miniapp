...
REPLY_WINDOW = 90 * 60
LATE_AUTO_HOURS = 24
MODE_LABELS = {"br": "بتل رویال", "casual": "کژوال", "resurgence": "ریسرجنس", "ranked": "رنک"}
...
def modes_text(lobby):
    modes = (lobby or {}).get("modes") or []
    names = [MODE_LABELS.get(m, m) for m in modes]
    return " / ".join(names) if names else "نامشخص"


def keyboard_for(lobby):
    L = labels_for(lobby)
    lid = sid((lobby or {}).get("id") or "x")[:16]
    return {"inline_keyboard": [[{"text": L["5"], "callback_data": f"a:{lid}:5"}, {"text": L["15"], "callback_data": f"a:{lid}:15"}], [{"text": L["30"], "callback_data": f"a:{lid}:30"}, {"text": L["60"], "callback_data": f"a:{lid}:60"}], [{"text": L["no"], "callback_data": f"a:{lid}:no"}], [{"text": "باز کردن تابلو", "url": "https://t.me/warzonesquad_bot/squad"}]]}
...
def notice_key(lobby):
    return sid((lobby or {}).get("id") or (lobby or {}).get("startIso") or "x").replace(".", "_").replace(":", "-")
...
def is_banned(uid):
    return bool(db_get(f"banned/{sid(uid)}"))
...
def member_ids():
    banned = {sid(x) for x in (db_get("banned") or {}).keys()}
    return [sid(uid) for uid in members_map().keys() if sid(uid) not in banned]
    ...
    if isinstance(data, dict) and data:
        ...
        for k, v in data.items():
            if not v:
                continue
            row = dict(v)
            row["id"] = sid(row.get("id") or k)
            out[row["id"]] = row
        return out
    old = db_get("lobby")
    if old and old.get("hostId"):
        row = dict(old)
        row["id"] = "legacy"
        row.setdefault("modes", ["br", "casual", "resurgence", "ranked"])
        return {"legacy": row}
    return {}


def save_lobby(lobby):
    lid = sid((lobby or {}).get("id"))
    if lid:
        db_put(f"lobbies/{lid}", lobby)
...
def notify_new_request(lobby, old):
    if not lobby or not lobby.get("hostId"):
        return
    if old and sid(old.get("hostId")) == sid(lobby.get("hostId")) and old.get("startIso") == lobby.get("startIso"):
        return
...
def notify_new_answers(lobby, old):
    if not lobby or not old or sid(lobby.get("id")) != sid(old.get("id")):
        return
    L = labels_for(lobby)
...
def tally_noreply(old, new):
    if not old or not old.get("hostId") or old.get("noreplyTallied"):
        return
    if new and sid(new.get("id")) == sid(old.get("id")):
        return
...
def maybe_remind(lobby):
    ...
    text = f"۱۰ دقیقه تا شروع {modes_text(lobby)} — {lobby.get('hostName', 'یکی')} درخواست داده بود."
...
def handle_update(upd):
    ...
    if "message" in upd and str(upd["message"].get("text") or "").startswith("/start"):
        user = upd["message"].get("from") or {}
        if is_banned(user.get("id")):
            tg("sendMessage", {"chat_id": upd["message"]["chat"]["id"], "text": "دسترسی این اکانت بسته شده."})
            return
        save_member(user)
    ...
    if data.startswith("lateadm:") or data.startswith("late:"):
        ...
        if kind == "late":
            ...
            name = "بازیکن"
            for lobby in all_lobbies().values():
                for x in lobby.get("lates") or []:
                    if sid(x.get("id")) == sid(target):
            ...
            try:
                ...
                pass
        return
    if not data.startswith("a:") and not data.startswith("ans:"):
        return
    parts = data.split(":")
    if data.startswith("a:") and len(parts) >= 3:
        lid, code = parts[1], parts[2]
    else:
        lid, code = "", parts[-1]
    user = q.get("from") or {}
    save_member(user)
    chat_id = q["message"]["chat"]["id"]
    lobbies = all_lobbies()
    lobby = lobbies.get(lid) if lid else (next(iter(lobbies.values())) if len(lobbies) == 1 else None)
    L = labels_for(lobby or {})
    if not lobby or not lobby.get("hostId"):
...
def main():
    ...
    while True:
        try:
            ...
            if now - last_check >= 3:
                cur = all_lobbies()
                ids = set(cur) | set(last)
                for lid in ids:
                    old_l = last.get(lid)
                    new_l = cur.get(lid)
                    if json.dumps(old_l, sort_keys=True) != json.dumps(new_l, sort_keys=True):
                        tally_noreply(old_l, new_l)
                        if new_l:
                            notify_new_request(new_l, old_l)
                            notify_new_answers(new_l, old_l)
                            notify_new_lates(new_l, old_l)
                for lobby in list(cur.values()):
                    maybe_tally_open(lobby)
                    auto_accept_lates(lobby)
                    maybe_remind(lobby)
                last = all_lobbies()
                last_check = now
            payload = {"timeout": 2}
